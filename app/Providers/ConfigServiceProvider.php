...
class ConfigServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        try {
            ...
            // zaincash config 
            $data = BusinessSetting::where(['key' => 'zaincash'])->first();
            $zain = json_decode($data['value'], true);
            if ($zain) {
                $config = array(
                    'zain_msisdn' => env('ZAIN_MSISDN', $zain['zain_msisdn']),
                    'zain_secret' => env('ZAIN_SECRET', $zain['zain_secret']),
                    'zain_merchantid' => env('ZAIN_MERCHANT_ID', $zain['zain_merchantid']),
                );
                Config::set('zain', $config);
            }
